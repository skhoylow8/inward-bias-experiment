...
const MAIN_INSTRUCTIONS_DICT = {
    ...
    2: [HIDE_INSTR_IMG, false, 'In this study, we will show you '+INSTR_TRIAL_N+' images, one at a time. We are interested in how positive you feel looking at each image.'],
...
function TRIAL_UPDATE(formal_trial, last, this_trial, next_trial, path) {
    ...
    trial.imagAdj = 0 //initialize rating just incase they don't move the image
}
...
function AQ_RESPONSE(event) {
    const RESP = $('input[name="rating"]:checked').val();
    if (CHECK_IF_RESPONDED([], [RESP])) {
        ...
        $('#aq-warning').text('Please answer the question to continue the experiment. Thank you!');
    }
}
...
const AQ_QUESTION_DICT = {
    ...
    // 13: "I would srather go to a library than to a party.",
